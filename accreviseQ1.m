...
Q=  Q+10.5*eye(4);  %K_o in the paper%
...
%            cframept(:,8) = [2.5;1;-1;1];
%             cframept(:,9) = [3.1;1;1;1];
%             cframept(:,10) = [3.7;0.5;0.5;1];
%             cframept(:,16) = [2.5;1.2;-1;1];
%              cframept(:,17) = [3;1;0.5;1];
...
%                       scatter3(y(1,j),y(2,j),y(3,j),40,'filled','Marker','o','MarkerFaceColor',color_covered_landmark_face/255,'MarkerEdgeColor',color_covered_landmark_edge/255,'LineWidth',1);
                      ...
                      omega2(:,:,i) = omega2(:,:,i)+alpha*(SE_3s(:,:,i)*(cframept(:,j)+deltay(:,j,i))-[y(1,j) y(2,j) y(3,j) 1].')*transpose([y(1,j) y(2,j) y(3,j) 1].')/landmarks; 
                      counter = counter+1;
...
%%%%%%%%%    Observer update       
        ...
        se_3s(:,:,i) =  U(:,:,i)-inv(SE_3s(:,:,i))*omega(:,:,i)*SE_3s(:,:,i)-inv(SE_3s(:,:,i))*projection (1/4*((Q*omega2(:,:,i) - omega2(:,:,i).'*Q.')))*SE_3s(:,:,i); 
